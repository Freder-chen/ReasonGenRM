...
class ReasonDPOTrainer(ABC):
    ...
    def fit(self, args, consumed_samples=0, num_update_steps_per_epoch=None):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for data in self.train_dataloader:
                ...
                #     chosen_ids, c_mask, reject_ids, r_mask, info = data
                ...
                #         self.model, chosen_ids, c_mask, reject_ids, r_mask, info["input_length"]
                #     )
                #     with torch.no_grad():
                #         reference_chosen_logps, reference_rejected_logps, _, _ = self.concatenated_forward(
                #             self.ref_model, chosen_ids, c_mask, reject_ids, r_mask, info["input_length"]
                #         )
                # else:
                assert self.packing_samples

                packed_input_ids, packed_attention_masks, packed_seq_lens, info = data
                packed_input_ids, packed_attention_masks = packed_input_ids.to(torch.cuda.current_device()), packed_attention_masks.to(torch.cuda.current_device())
                chosen_logps, rejected_logps, aux_loss, nll_loss = self.packed_samples_forward(
                    ...
                    packed_seq_lens,
                    info["input_length"],
                    info["chosen_reason_suffix_ids_range"],
                    info["rejected_reason_suffix_ids_range"]
                )
                with torch.no_grad():
    ...
    def evaluate(self, eval_dataloader, steps=0):
        ...
        with torch.no_grad():
            ...
            for data in eval_dataloader:
                ...
                packed_input_ids, packed_attention_masks, packed_seq_lens, info = data
                packed_input_ids, packed_attention_masks = packed_input_ids.to(torch.cuda.current_device()), packed_attention_masks.to(torch.cuda.current_device())
                chosen_logps, rejected_logps, _, _ = self.packed_samples_forward(
                    self.model,
                    packed_input_ids,
                    packed_attention_masks,
                    packed_seq_lens,
    ...
    def packed_samples_forward(
        ...
        packed_seq_lens,
        prompt_id_lens,
        chosen_reason_suffix_ids_ranges,
        rejected_reason_suffix_ids_ranges,
    ):
        output = model(
            packed_input_ids,
        ...
        all_logps_sum, all_logps_mean = self._packed_get_batch_logps(
            ...
            packed_attention_masks,
            prompt_id_lens * 2,
            chosen_reason_suffix_ids_ranges + rejected_reason_suffix_ids_ranges,
            packed_seq_lens,
    ...
    def _packed_get_batch_logps(
        ...
        for i, seq_len in enumerate(packed_seq_lens):
            ...
            loss_masks[0, index + reason_suffix_ids_ranges[i][0] : index + reason_suffix_ids_ranges[i][1]] = False
            index = index + seq_len
